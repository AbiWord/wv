...
void
decode_f_reference (textportions * portions)
{
    U32 i;
    ...
    while (i < portions->fndref_no)
	  ...
	  if (portions->fndRef[i] == cp)
	    {
		error (erroroutput,
		       "found correct ref pos at %x, index was %d\n",
		       portions->fndRef[i], i);
		if (portions->fndFRD[i].frd > 0)
		  {
		      if (!insuper)
			{
		      ...
		      decode_list_nfc (portions->fndFRD[i].frd,
				       dop.new_nfcFtnRef);
	  ...
	  error (erroroutput, "conpared given pos %x and ref list %x", cp,
		 portions->fndRef[i]);
	  i++;
      }
...
void
decode_e_reference (textportions * portions)
{
    int i;
		...
		if (portions->endFRD[i].frd > 0)
		  {
		      if (!insuper)
			{
		      ...
		      error (erroroutput,
			     "is autonumbered <%d>, i is %d, true is %d\n",
			     portions->endFRD[i].frd, i,
			     portions->endTrueFRD[i]);
		      portions->endTrueFRD[i] = portions->auto_end++;
		      fprintf (outputfile, "<a href=\"#end%d\">", i);
		      decode_list_nfc (portions->endTrueFRD[i],
				       dop.new_nfcEdnRef);
		      fprintf (outputfile, "</a>");
		  }
		else
