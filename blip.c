...
#if 0
char *
wvGetBitmap (BitmapBlip * abm, MSOFBH * amsofbh, FBSE * afbse, wvStream * infd)
{
    return ("/tmp/rubbish");
    char *aimage;
    char *buffer = NULL;
...
#if 0
char *
wvGetMetafile (MetaFileBlip * amf, MSOFBH * amsofbh, wvStream * infd)
{
    return ("/tmp/rubbish");
#if 0
    char *aimage;
...
#if 0
fsp_list *
wvParseEscher (fbse_list ** pic_list, U32 fcDggInfo, U32 lcbDggInfo,
	       wvStream * tablefd, FILE * mainfd)
{
    int remainder;
    U32 i, j;
    MSOFBH amsofbh;
    ...
    wvTrace (
	     ("the id here is %x, loc %x, ends at %x\n", rmsofbh.fbt,
	      ftell (out), ftell (out) + rmsofbh.cbLength));
    ...
    while ((U32) (ftell (out)) < lcbDggInfo)
	  ...
	  wvTrace (
		   ("the id here is %x, loc %x, ends at %x\n", amsofbh.fbt,
		    ftell (out), ftell (out) + amsofbh.cbLength));
	  switch (amsofbh.fbt)
	    {
	    case msofbtDgg:
		wvGetFDGG (&afdgg, out);
		...
		if (afbse_list == NULL)
		  ...
		  {
		      pfbse_list->next =
			  (fbse_list *) wvMalloc (sizeof (fbse_list));
		...
		if (wvQueryDelayStream (&(pfbse_list->afbse)))
		  {
		      ...
		      if (pfbse_list->afbse.cRef)
			{
			    ...
			    if (pfbse_list->afbse.foDelay < dtest)
			      {
				  fseek (out, pfbse_list->afbse.foDelay,
					 SEEK_SET);
				  dtest = 0;
			      }
			    else
		...
		if (ftell (out) != finish)
		  {
		      wvTrace (
			       ("file is out of sync, pos is %x ended at %x\n",
				ftell (out), finish));
		...
		if (afsp_list == NULL)
		  ...
		  {
		      pfsp_list->next =
			  (fsp_list *) wvMalloc (sizeof (fsp_list));
		...
		while (remainder >= 6)
		  {
		      if (i == 0)
			{
			    if (pfsp_list == NULL)
			      {
				  wvTrace (
					   ("must be a top level property list\n"));
				  pfsp_list =
				      (fsp_list *) wvMalloc (sizeof (fsp_list));
			      }
			    pfsp_list->afopte_list =
				(fopte_list *) wvMalloc (sizeof (fopte_list));
