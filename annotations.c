...
void
decode_annotation (textportions * portions)
{
    U32 i, j, val;
    /*
    ...
    while (i < portions->andref_no)
	  ...
	  if (portions->andRef[i] == (U32) cp)
	    {
		error (erroroutput,
		       "found correct anno ref pos at %x, index was %d\n",
		       portions->andRef[i], i);
		if (!insuper)
		...
		for (j = 1; j < val; j++)
		  {
		      ...
		      fprintf (outputfile, "%c",
			       portions->the_atrd[i].xstUsrInitl[j]);
		  }
		fprintf (outputfile, "%d\">[", i);
		j = portions->the_atrd[i].xstUsrInitl[0];
		val = portions->the_atrd[i].xstUsrInitl[0] + 1;
		key_atrd[portions->the_atrd[i].ibst].xstUsrInitl[0] = j;
		key_atrd[portions->the_atrd[i].ibst].ibst =
		    portions->the_atrd[i].ibst;
		for (j = 1; j < val; j++)
		  {
		      /*warning despite the possibility of being 16 bit nos ive done this */
		      fprintf (outputfile, "%c",
			       portions->the_atrd[i].xstUsrInitl[j]);
		      key_atrd[portions->the_atrd[i].ibst].xstUsrInitl[j] =
			  portions->the_atrd[i].xstUsrInitl[j];
		  }
    ...
    if (lcbGrpXstAtnOwners > 0)
	  ...
	  while (count < lcbGrpXstAtnOwners)
		...
		if (current->u16string == NULL)
		  {
		      fprintf (erroroutput,
			       "not enough mem for author string of len %d\n",
			       len);
		      break;
		  }
		for (i = 0; i < len; i++)
		...
		if (count < lcbGrpXstAtnOwners)
		  {
			...
			{
			    fprintf (erroroutput,
				     "not enough mem for annotation group\n");
			    break;
			}
...
U32
decode_b_annotation (bookmark_limits * l_bookmarks)
{
    U32 i = 0;
    while (i < l_bookmarks->bookmark_b_no)
	  ...
	  if (l_bookmarks->bookmark_b_cps[i] == (U32) realcp)
		...
		fprintf (outputfile, "\">");
		fprintf (outputfile, "<img src=\"%s/doccommentb.jpg\">",
			 patterndir ());
    ...
    while (i < l_bookmarks->bookmark_b_no)
      {
	  if ((l_bookmarks->bookmark_b_cps[i] != 0xffff)
	      && (l_bookmarks->bookmark_b_cps[i] > (U32) realcp))
...
U32
decode_e_annotation (bookmark_limits * l_bookmarks)
{
    U32 i = 0;
    ...
    while (i < l_bookmarks->bookmark_e_no)
	  ...
	  if (l_bookmarks->bookmark_e_cps[i] == (U32) realcp)
		...
		fprintf (outputfile, "<img src=\"%s/doccommente.jpg\">",
			 patterndir ());
    ...
    while (i < l_bookmarks->bookmark_e_no)
      {
	  if ((l_bookmarks->bookmark_e_cps[i] != 0xffff)
	      && (l_bookmarks->bookmark_e_cps[i] > (U32) realcp))
