...
void
wvGetPropHeader (PropHeader * header, wvStream * file)
{
    int i;
...
void
wvGetFIDAndOffset (FIDAndOffset * fid, wvStream * file)
{
    int i;
    for (i = 0; i < 4; i++)
...
void
wvReleaseSummaryInfo (SummaryInfo * si)
{
    if (si == NULL)
	return;
    if (si->aProps != NULL)
...
void
wvGetSummaryInfo (SummaryInfo * si, wvStream * file, U32 offset)
{
    U32 i;
...
*/
int
wvSumInfoForceString (char *lpStr, U16 cbStr, U32 pid, SummaryInfo * si)
{
    int len;
    PropValue Prop;
...
int
wvSumInfoGetString (char *lpStr, U16 cbStr, U32 pid, SummaryInfo * si)
{
    int len;
    PropValue Prop;
    if (1 == wvGetProperty (&Prop, si, pid))
...
int
wvSumInfoGetPreview (char *lpStr, U16 cbStr, U32 pid, SummaryInfo * si)
{
    int len, i;
    PropValue Prop;
...
int
wvSumInfoGetLong (U32 * lpLong, U32 pid, SummaryInfo * si)
{
    int ret;
    PropValue Prop;
...
int
wvSumInfoGetTime (U16 * yr, U16 * mon, U16 * day, U16 * hr, U16 * min,
...
int
wvGetProperty (PropValue * Prop, SummaryInfo * si, U32 pid)
{
    U32 i, j;
    U8 *t;
		...
		switch (Prop->vtType)
		  ...
		  case VT_WMF:
		      ...
		      if (Prop->vtValue.vtBSTR.cBytes == 0)
			{
		      ...
		      Prop->vtValue.vtBSTR.ch =
			  (char *) wvMalloc (Prop->vtValue.vtBSTR.cBytes);
...
int
wvSumInfoOpenStream (SummaryInfo * si, wvStream * stream)
{
    PropHeader header;
    FIDAndOffset fid;
    U32 i;
    ...
    for (i = 0; i < header.cSections; i++)
	  ...
	  if (fid.dwords[0] == 0XF29F85E0 &&
	      ...
	      fid.dwords[2] == 0X000891AB && fid.dwords[3] == 0XD9B3272B)
	      break;
      }
#ifdef DEBUG
...
int
wvOLESummaryStream (char *filename, wvStream ** summary)
{
    int ret;
